...
void extIntEnableIrq(void)
{
   //Enable PIOA interrupts
	XIntc_Enable(&InterruptController, XPAR_AXI_INTC_0_SYSTEM_INT_NIC100_INTR);
	IRQenabled = true;
...
void extIntDisableIrq(void)
{
   //Disable PIOA interrupts
	XIntc_Disable(&InterruptController, XPAR_AXI_INTC_0_SYSTEM_INT_NIC100_INTR);
	IRQenabled = false;
}
...
{
   XIntc_Acknowledge(&InterruptController, XPAR_AXI_INTC_0_SYSTEM_INT_NIC100_INTR);
   bool_t flag;
