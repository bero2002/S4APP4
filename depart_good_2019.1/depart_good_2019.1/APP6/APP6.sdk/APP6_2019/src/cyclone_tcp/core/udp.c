   ...
   IpPseudoHeader *pseudoHeader, const NetBuffer *buffer, size_t offset)
{
   ...
   header = (UdpHeader*)netBufferAt(buffer, offset);
   //Sanity check
   if(header == NULL)
   ...
   for(i = 0; i < SOCKET_MAX_COUNT; i++)
   {
      ...
      //UDP socket found?
      if(socket->type != SOCKET_TYPE_DGRAM){//print("Type");
         continue;}
      //Check whether the socket is bound to a particular interface
      if(socket->interface && socket->interface != interface){//print("Bound");
         continue;}
      //Check destination port number
      if(socket->localPort == 0 || socket->localPort != ntohs(header->destPort)){//print("Dest");
         continue;}
      //Source port number filtering
      if(socket->remotePort != 0 && socket->remotePort != ntohs(header->srcPort)){//print("Source");
         continue;}
   ...
   if(!socket->receiveQueue)
   {
      ...
      if(p != NULL)
      {
         ...
         queueItem = (SocketQueueItem*)netBufferAt(p, 0);
         queueItem->buffer = p;
   ...
   else
   {
      ...
      {
         //Add the newly created item to the queue
         queueItem->next = (struct _SocketQueueItem*)netBufferAt(p, 0);
         //Point to the newly created item
         queueItem = queueItem->next;
         queueItem->buffer = p;
      }
